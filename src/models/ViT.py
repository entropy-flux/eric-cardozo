...
class FFN(Module): 
    ...
    def forward(self, sequence: Tensor) -> Tensor: 
        ...
        sequence = relu(self.z_projector(sequence))
...
class CLSToken(Module):
    def __init__(
        ...
        super().__init__()
        self.tokens = Parameter(zeros(1, 1, model_dimension)) 

    def forward(self, sequence: Tensor) -> Tensor:
        sequence = sequence.flatten(2).transpose(1, 2) 
        return concat((self.tokens.expand(sequence.size(0), -1, -1), sequence), dim=1) 
...
class ViT(Module):  
    def __init__(
        ...
        self.labeler = CLSToken(model_dimension) 
